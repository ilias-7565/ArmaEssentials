...
if (hasInterface) then
{
	...
	[_object] spawn
	{
		...
		if ([_object] call AdvLog_fnc_canMove) then
		{
			...
			_myaction = ["R3F_move", format [STR_R3F_LOG_action_move_object, _name],"\r3fAdvLog\data\box_carry.paa",{_this spawn AdvLog_fnc_moveObj;},{!R3F_LOG_mutex_local_lock},{}] call ace_interact_menu_fnc_createAction;
		...
		{
			if ([_object] call AdvLog_fnc_canETow && _object isKindOf "LandVehicle") then
			{
		...
		if (([_object] call AdvLog_fnc_canBeTransported) select 0) then
		{
			...
			_myaction = ["R3F_load", format [STR_R3F_LOG_action_select_object_load, _name],"\r3fAdvLog\data\Icon_load.paa",{_this call AdvLog_fnc_selected;},{!R3F_LOG_mutex_local_lock},{}] call ace_interact_menu_fnc_createAction;
